...
module.exports = (options) => {
  return {
    ...
    async transformIndexHtml(html, { path: indexPath, context, filename }) {
      ...
      const imgOutputPath = normalizePath(
        path.posix.join(projectPath, imgOutputDir)
      ); // ex. /Users/myuser/Documents/boilerplate-vite-image-plugin/dist/
      // Get the directory path that the currently processed HTML file is in
      const currentHTMLdir = normalizePath(
        path.posix.dirname(path.posix.join(outputPath, currentFilePath))
      ); // ex. /Users/myuser/Documents/boilerplate-vite-image-plugin/dist
        ...
        try {
          ...
          for (const imgTag of imgTags) {
            try {
              ...
              const currentIMGpath = normalizePath(
                path.posix.join(imgOutputPath, src)
              );
              const outputImagePath = normalizePath(
                path.posix.relative(currentHTMLdir, currentIMGpath)
              );

              // Construct the full path to the input image based on /src/assets/images
              const inputImagePath = normalizePath(
                path.posix.resolve(imgInputDir, src)
              );

              // Update the src attribute of the <img> tag with the cleaned relative path
              imgTag.setAttribute("src", outputImagePath);

              // If the image tag does not have a loading attribute, set it to lazy
              if (!imgTag.hasAttribute("loading")) {
              ...
              if (!src || !/\.(jpg|png|jpeg)$/.test(src)) {
                ...
                if (
                  ...
                  configCommand === "build" &&
                  !processedImages.has(inputImagePath)
                ) {
                  // Add the input image path to the processed images Set
                  processedImages.set(inputImagePath, []);

                  // Copy the original image to the output directory
                  const outputImageCopyPath = normalizePath(
                    path.posix.resolve(imgOutputDir, src)
                  );
                    ...
                    const imagePromises = sizesToGenerate.map(async (size) => {
                      if (size <= originalWidth) {
                        ...
                        const webpBuffer = await image
                          ...
                          .toFormat("webp")
                          .toBuffer();
                        const webpFileName = `${path.basename(
                          src,
                          path.extname(src)
                        )}-${size}px.webp`;
                        ...
                        const outputImagePath = normalizePath(
                          path.posix.resolve(imgOutputDir, webpFileName)
                        );
                ...
                async function generatePictureTags(inputImagePath) {
                  ...
                  let imgPath = (htmlDir, imgName, imgDir) => {
                    return normalizePath(
                      path.relative(htmlDir, path.resolve(imgDir, imgName))
                    );
                  };
                  ...
                  if (!imgSizes) {
                    console.error(
                      `No processed images found for ${inputImagePath}`
                    );
                    return;
                  }

                  let outputString = imgSizes
                    .map((image) => {
                      const srcSet = `${imgPath(
                        currentHTMLdir,
                        image.src,
                        imgOutputDir
                      )} ${image.size}w`;
              ...
              console.error(
                `Error processing image ${imgTag.getAttribute("src")}: ${
                  error.message
                }`
              );
            }
